...
static void inputGetKeys_default_descriptor(void)
{
   #define buttons_map(PAD) \
    ...
    { PAD, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_SELECT, "C-Down" },\
...
static void process_analog_input(int16_t analogX, int16_t analogY, BUTTONS* Keys)
{
   double radius, angle;
...
{
   // Process left analog stick (Control Stick)
   ...
   process_analog_input(analogX, analogY, Keys);

   // Only process right analog stick if the left analog stick returned (0, 0)
...
void inputGetKeys_default( int Control, BUTTONS *Keys )
{
   ...
   if (libretro_supports_bitmasks)
      ret = input_cb(Control, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_MASK);
   else
   {
   ...
   Keys->A_BUTTON     = !!((ret & (1 << RETRO_DEVICE_ID_JOYPAD_A)));
   Keys->B_BUTTON     = !!((ret & (1 << RETRO_DEVICE_ID_JOYPAD_B)));
   Keys->R_CBUTTON    = !!((ret & (1 << RETRO_DEVICE_ID_JOYPAD_R2)));
   Keys->L_CBUTTON    = !!((ret & (1 << RETRO_DEVICE_ID_JOYPAD_Y)));
   Keys->D_CBUTTON    = !!((ret & (1 << RETRO_DEVICE_ID_JOYPAD_SELECT)));
